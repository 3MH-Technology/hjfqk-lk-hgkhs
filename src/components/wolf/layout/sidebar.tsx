...
/* ─── Status dot types ─── */
type StatusDot = 'green' | 'blue' | 'none';
...
function StatusDotIndicator({ color }: { color: StatusDot }) {
  ...
  return (
    <span className="relative flex size-2 shrink-0">
      {color === 'green' && (
        <span className={`absolute inline-flex h-full w-full animate-ping rounded-full ${pingClasses[color]} opacity-60`} />
      )}
      <span className={`relative inline-flex size-2 rounded-full ${colorClasses[color]} ${color === 'blue' ? 'ring-2 ring-blue-400/30' : ''}`} />
    </span>
  );
}
...
function SectionLabel({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-2 px-3 pt-3 pb-1.5">
      ...
      <div className="flex-1 h-px bg-sidebar-foreground/8" />
    </div>
  );
}
  ...
  return (
    <div className="px-3 pb-2 relative">
      <div className="relative">
        <Search className="absolute right-2.5 top-1/2 -translate-y-1/2 size-3.5 text-sidebar-foreground/30" />
        <input
          type="text"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder="بحث سريع..."
          className="w-full h-8 rounded-md bg-sidebar-accent/60 border border-sidebar-border/50 pl-3 pr-8 text-xs text-sidebar-foreground placeholder:text-sidebar-foreground/30 focus:outline-none focus:ring-1 focus:ring-primary/30 focus:border-primary/30 transition-all"
          dir="rtl"
        />
        {value.length > 0 && (
          <button
            onClick={() => onChange('')}
            className="absolute left-2 top-1/2 -translate-y-1/2 text-sidebar-foreground/30 hover:text-sidebar-foreground/60 transition-colors"
          >
            <X className="size-3" />
          </button>
        )}
      ...
      <AnimatePresence>
        {value.length > 0 && results.length > 0 && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg bg-popover border border-border/50 shadow-xl overflow-hidden max-h-48 overflow-y-auto"
          >
            {results.map((item) => (
              <button
                key={item.id}
                ...
                className="flex items-center gap-2 w-full px-3 py-2 text-xs text-popover-foreground hover:bg-accent/80 transition-colors text-right"
              >
...
function ShortcutBadge({ shortcut }: { shortcut?: string }) {
  ...
  return (
    <kbd className="hidden lg:inline-flex items-center gap-0.5 rounded border border-sidebar-foreground/10 bg-sidebar-accent/50 px-1.5 py-0.5 text-[9px] font-medium text-sidebar-foreground/30 ml-auto shrink-0">
      {shortcut}
    </kbd>
  );
}
...
function SidebarContent({
  ...
  return (
    <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
      ...
      {/* Navigation */}
      <nav className="flex-1 overflow-y-auto px-3 py-1" role="navigation">
              ...
              {section.items.map((item) => {
                ...
                const button = (
                  ...
                  >
                    ...
                    {/* Status dot */}
                    {item.statusDot && item.statusDot !== 'none' && (
                      <StatusDotIndicator color={item.statusDot} />
                    )}
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed right-0 top-0 z-50 h-screen w-72 md:hidden shadow-2xl shadow-black/40"
            >
              <SidebarContent
