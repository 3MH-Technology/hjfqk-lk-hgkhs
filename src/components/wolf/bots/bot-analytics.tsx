...
const performanceData = [
  { day: 'السبت', requests: 12400, responseTime: 142, errors: 47, cpu: 45, memory: 58 },
  { day: 'الأحد', requests: 15200, responseTime: 128, errors: 32, cpu: 52, memory: 62 },
  { day: 'الاثنين', requests: 18600, responseTime: 165, errors: 68, cpu: 68, memory: 71 },
  { day: 'الثلاثاء', requests: 14300, responseTime: 138, errors: 41, cpu: 48, memory: 60 },
  { day: 'الأربعاء', requests: 16800, responseTime: 155, errors: 56, cpu: 58, memory: 65 },
  { day: 'الخميس', requests: 19500, responseTime: 172, errors: 73, cpu: 72, memory: 74 },
  { day: 'الجمعة', requests: 11200, responseTime: 119, errors: 28, cpu: 38, memory: 52 },
];

// Resource distribution
const resourceData = [
  { name: 'المعالج (CPU)', value: 32, color: '#34d399' },
  { name: 'الذاكرة (RAM)', value: 28, color: '#f59e0b' },
  { name: 'الشبكة', value: 22, color: '#818cf8' },
  { name: 'القرص', value: 18, color: '#fb923c' },
];

// Response time distribution
const responseTimeBuckets = [
  { range: '0-100ms', count: 8420, fill: '#34d399' },
  { range: '100-500ms', count: 5120, fill: '#f59e0b' },
  { range: '500ms-1s', count: 1530, fill: '#fb923c' },
  { range: '>1s', count: 430, fill: '#ef4444' },
];

// Bot health data
const botHealthData = [
  { id: '1', name: 'بوت الدعم الفني', status: 'healthy' as const, uptime: 99.8, reqPerMin: 42, errorCount: 12 },
  { id: '2', name: 'بوت الإشعارات', status: 'healthy' as const, uptime: 99.5, reqPerMin: 28, errorCount: 8 },
  { id: '3', name: 'بوت المبيعات', status: 'warning' as const, uptime: 94.2, reqPerMin: 65, errorCount: 47 },
  { id: '4', name: 'بوت الإدارة', status: 'healthy' as const, uptime: 99.9, reqPerMin: 15, errorCount: 3 },
  { id: '5', name: 'بوت التحليلات', status: 'critical' as const, uptime: 78.5, reqPerMin: 8, errorCount: 156 },
  { id: '6', name: 'بوت المحادثة', status: 'warning' as const, uptime: 91.3, reqPerMin: 53, errorCount: 62 },
];

// Top error messages
const topErrors = [
  { message: 'ETIMEDOUT: انتهاء مهلة الاتصال بقاعدة البيانات', count: 156, severity: 'critical' as const },
  { message: 'ECONNREFUSED: تم رفض الاتصال بخدمة Redis', count: 89, severity: 'critical' as const },
  { message: 'TypeError: لا يمكن قراءة خاصية "id" من undefined', count: 67, severity: 'warning' as const },
  { message: 'Error: تجاوز حد معدل الطلبات (Rate Limit)', count: 54, severity: 'warning' as const },
  { message: 'FETCH_ERROR: فشل في إرسال إشعار تيليجرام', count: 41, severity: 'critical' as const },
  { message: 'ValidationError: بيانات إدخال غير صالحة', count: 38, severity: 'warning' as const },
  { message: 'ENOMEM: نفاد الذاكرة - حظر جديد غير ممكن', count: 23, severity: 'critical' as const },
  { message: 'SyntaxError: خطأ في تحليل JSON', count: 19, severity: 'warning' as const },
];
...
function generateHeatmapData(): number[][] {
  ...
  for (let day = 0; day < 7; day++) {
    ...
    for (let hour = 0; hour < 24; hour++) {
      // Simulate realistic pattern: low at night, peak during work hours
      const hourFactor = hour >= 8 && hour <= 22 ? 0.6 : 0.1;
      const weekendFactor = day >= 5 ? 0.5 : 1;
      const base = Math.random() * hourFactor * weekendFactor;
        ...
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-muted/30 rounded-lg p-2 text-center">
              <p className={`text-sm font-bold tabular-nums ${bot.uptime >= 95 ? 'text-emerald-400' : bot.uptime >= 85 ? 'text-amber-400' : 'text-red-400'}`}>
                {bot.uptime}%
              </p>
            ...
            <div className="bg-muted/30 rounded-lg p-2 text-center">
              <p className={`text-sm font-bold tabular-nums ${bot.errorCount > 50 ? 'text-red-400' : bot.errorCount > 20 ? 'text-amber-400' : 'text-emerald-400'}`}>
                {bot.errorCount}
              </p>
  ...
  return (
    ...
    >
      <div className={`mt-1 shrink-0 flex items-center justify-center size-6 rounded-full ${
        error.severity === 'critical' ? 'bg-red-500/15' : 'bg-amber-500/15'
      }`}>
...
const requestSparkData = [8200, 9100, 8800, 10200, 9500, 11300, 10800];
const responseSparkData = [165, 142, 158, 128, 155, 138, 119];
const errorSparkData = [52, 47, 62, 32, 56, 41, 28];
const uptimeSparkData = [99.2, 99.4, 98.8, 99.5, 99.1, 99.3, 99.8];
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                ...
                <div className="flex gap-0.5">
                  ...
                  <span className="size-3 rounded-sm bg-amber-500/50" />
